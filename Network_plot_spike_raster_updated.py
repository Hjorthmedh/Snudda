...
class NetworkPlotSpikeRaster(object):
  ...
  def plotColourRaster(self,skipTime,plotIdx="sort",typeOrder=None):
    ...
    #print(plotIdx,tickPos,tickText)
    ...
    lineStart = 0
    for t in typeOrder:
        pruned_spikes = [   self.time[int(s)] \
                            for s in typedict[t] \
                            if cellTypes[int(s)] == t and int(s) in tIdx]
        pruned_cell   = [   i+lineStart \
                            for i,s in enumerate(typedict[t]) \
                            if cellTypes[int(s)] == t and int(s) in tIdx]
        lineStart += len(typedict[t])
        if t == 'dSPN': denc = True #; print(pruned_spikes)
        else: denc = False
        c=colours[t] 
        ax.scatter(pruned_spikes,
                   pruned_cell,
                   color=c,
                   s=1,
                   linewidths=0.1) 
    ...
    #plt.ion()
    ax.set_ylim([-100,len(self.networkInfo.data["neurons"])+100])
    ax.invert_yaxis()
    figName = 'figs/Network-spike-raster-{}-colour-skip{}.png'.format(self.ID, int(skipTime*1e3))
  ...
  def sortTraces(self,typeOrder=None):
    ...
    for t in typeOrder:
        typedict[t] = [ i for i,x in enumerate(allTypes) if x == t]
  ...
  def makePlotLookup(self,plotIdx):
    ...
    plotLookup = np.nan*np.zeros((np.max(self.spikeID)+2,))
