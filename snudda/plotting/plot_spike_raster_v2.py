...
class SnuddaPlotSpikeRaster2:
    ...
    def plot_hist_raster(self, type_order=None, skip_time=0, end_time=None, fig_size=None, type_division=None):
        ...
        #Spike rates
        for t in type_order:
            cellSpikeRates=np.array([])
            spikesOfType=[i for i, x in enumerate(spike2type) if x.lower() == t.lower()]
            pruned_spikes = self.spike_time[spikesOfType] - skip_time
            pruned_spikes = pruned_spikes[pruned_spikes > 0]
            spike_y_t=spike_y[spikesOfType]
            spike_y_t=spike_y_t[pruned_spikes > 0]
            uspike_y_t=np.unique(spike_y_t)
            for cellid in uspike_y_t:
                cellSpikeRate=len(pruned_spikes[cellid==spike_y_t])/(end_time - skip_time)
                cellSpikeRates=np.append(cellSpikeRates,cellSpikeRate)
            num_of_type = len(type_dict[t])
            cellSpikeRates=np.append(cellSpikeRates,np.zeros(num_of_type - len(cellSpikeRates)))
            #print("Mean: {0}Hz, Stdev: {1}Hz {2} {3}".format(t,np.mean(cellSpikeRates),np.std(cellSpikeRates),cellSpikeRates))
            print("NeuronType: {0}, Mean: {1}Hz Stdev: {2}Hz".format(t, np.mean(cellSpikeRates), np.std(cellSpikeRates)))
        ...
        for t in type_order:
            ...
            spikeRate=len(pruned_spikes) / num_of_type / (end_time - skip_time)
            print("{0}: {1} Hz".format(t,spikeRate))
