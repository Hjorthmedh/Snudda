...
class RegionMesh(object):
    ...
    def ray_casting_alt(self, point):
        ...
        for i in range(0, n_tri):
            status = self.ray_intersect_triangle(point, self.point_out, self.mesh_vec[self.mesh_faces[i, :], :])

            if status == 2:
                # On a triangle, ie inside
                return True

            intersect_count += status

        return np.mod(intersect_count, 2) == 1

    # Taken from: https://www.erikrotteveel.com/python/three-dimensional-ray-tracing-in-python/
