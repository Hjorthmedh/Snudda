...
JOBDIR=../networks/lateral_twopop_virtual
...
if [ "$SLURM_PROCID" -gt 0 ]; then
    ...
    python setup_network_two_population_virtual.py ${JOBDIR}
...
#    sleep 120 #60
    ...
    # echo ">>> Ablate: "`date`
    python ablate_network_virtual.py ${JOBDIR}
    
    echo ">>> Input: "`date`
    # snudda input ${JOBDIR} --parallel --time 5 --input input.json
    snudda input ${JOBDIR} --parallel --time 5 --input input.json --networkFile ${JOBDIR}/network-synapses-virtual.hdf5
