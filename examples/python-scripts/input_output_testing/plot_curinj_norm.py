...
#plot pd2_norm
network_file_pd2 = os.path.join(network_path_pd2,"network-synapses.hdf5")
network_output_pd2_norm = os.path.join(network_path_pd2, "simulation", 'output_pd2_norm.hdf5')
pio_pd2_norm = PlotInputVsOutput(network_path_pd2, network_output_pd2_norm, network_file=network_file_pd2)
pio_pd2_norm.plot_cur_input_vs_output(experiment_config_file=experiment_config_file)
pt_pd2_norm = PlotTraces(network_output_pd2_norm, network_file=network_file_pd2)
pt_pd2_norm.plot_traces_sep()
pt_pd2_norm.plot_traces(fig_name="traces_pd2_norm.pdf")
...
cur_steps_pd2_norm, spike_rate_list_pd2_norm = pio_pd2_norm.get_spike_rate_list(experiment_config_file=experiment_config_file_norm)
avg_spike_rate_list_pd0 = np.mean(spike_rate_list_pd0,axis=0)
avg_spike_rate_list_pd2 = np.mean(spike_rate_list_pd2,axis=0)
avg_spike_rate_list_pd2_norm = np.mean(spike_rate_list_pd2_norm,axis=0)
...
ax.plot(cur_steps_pd2_norm, avg_spike_rate_list_pd2_norm, marker='.', linestyle='dashed',
        label='PD2_norm', color='red')#,color='black'
ax.plot(cur_steps_pd2, avg_spike_rate_list_pd2_norm, marker='.', linestyle='dotted',
        label='PD2_norm_remapped', color='red')#,color='black'
plt.legend()
